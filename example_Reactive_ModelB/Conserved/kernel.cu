...
void Allocate(Variable & Var, InputPara InputP)
{
	...
	CUDACHECK(cudaMallocManaged(&Var.change_x, size));
	CUDACHECK(cudaMallocManaged(&Var.change_y, size));
	CUDACHECK(cudaMallocManaged(&Var.change_z, size));
				...
				if (idx < Param.Nx && idy < Param.Ny && idz < Param.Nz)
				{
					...
					R = Param.k_0 * (exp(Param.n * Param.vx * dfdx[index] + Param.m * Param.vy * dfdy[index]) \
                        -exp(Param.vz * dfdz[index]));
                        		...
                        		double change_x_temp, change_y_temp, change_z_temp;
					change_x_temp = Param.dt * Param.vx * (Param.Mobility_x * Laplacian(dfdx, idx, idy, idz, Param) - Param.n * R );
					change_y_temp = Param.dt * Param.vy * (Param.Mobility_y * Laplacian(dfdy, idx, idy, idz, Param) - Param.m * R );
					change_z_temp = Param.dt * Param.vz * (Param.Mobility_z * Laplacian(dfdz, idx, idy, idz, Param) + R );
					
        				xnew[index] = xold[index] + change_x_temp;
        
        				ynew[index] = yold[index] + change_y_temp;
        
        				znew[index] = zold[index] + change_z_temp;
...
__global__ void Conservation(double* x_new, double* y_new, double* z_new, double* b, double change_x_avg, double change_y_avg, double change_z_avg, InputPara InputP)
{
	int x, y, z, index;
		...
		for (int j = 0; j < (InputP.Ny - 1) / t_tot_y + 1; j++)
			for (int i = 0; i < (InputP.Nx - 1) / t_tot_x + 1; i++)
			{
				...
				if (x < InputP.Nx && y < InputP.Ny && z < InputP.Nz)
				{
					...
					x_new[index] -= change_x_avg;
					y_new[index] -= change_y_avg;
					z_new[index] -= change_z_avg;
...
__global__ void Update_wakeup(double* xnew, double* ynew, double* znew, double* change_x, double* change_y, double* change_z, double* phinew, \
                        		double* xold, double* yold, double* zold, double* phiold, \
                        		double* b, \
                        		double* dfdx, double* dfdy, double* dfdz, \
						 		int* wake_up, InputPara Param)
			...
			for (int i = 0; i < (Param.Nx - 1) / t_tot_x + 1; i++)
			{
				...
				if (wake_up_s)		// Check if this block is awake
				{
					...
					if (idx < Param.Nx && idy < Param.Ny && idz < Param.Nz)
					{
						...
						ynew[index] = yold[index] + change_y_temp;
						znew[index] = zold[index] + change_z_temp;
						change_x[index] = change_x_temp;
						change_y[index] = change_y_temp;
						change_z[index] = change_z_temp;

					}
				}
				else	// Have to set change = 0 for inactive blocks
				{
					idz = blockIdx.z * blockDim.z + threadIdx.z + k * t_tot_z;
					idy = blockIdx.y * blockDim.y + threadIdx.y + j * t_tot_y;
					idx = blockIdx.x * blockDim.x + threadIdx.x + i * t_tot_x;
...
void FreeMemory(Variable & Var)
{
	...
	CUDACHECK(cudaFree(Var.change_x));
	CUDACHECK(cudaFree(Var.change_y));
	CUDACHECK(cudaFree(Var.change_z));
...
{	
    ...
    while (t <= InputP.t_total) {
		...
		else
		{
			...
			/*-------------------For performance test-------------------*/
			cudaEventRecord(kernel_start, 0);
			Update_wakeup << <dimGrid3D, dimBlock3D >> > (Var.x_new, Var.y_new, Var.z_new, Var.change_x, Var.change_y, Var.change_z, Var.phi_new, \
													Var.x_old, Var.y_old, Var.z_old, Var.phi_old, Var.b, \
													Var.dfdx, Var.dfdy, Var.dfdz,\
													Var.wake_up, InputP);
			cudaEventRecord(kernel_end, 0);
			...
			// Use Thrust to perform the reduction
    			...
    			thrust::device_ptr<double> dev_ptr_x(Var.change_x);
    			thrust::device_ptr<double> dev_ptr_y(Var.change_y);
    			thrust::device_ptr<double> dev_ptr_z(Var.change_z);
    			double change_x_sum = thrust::reduce(dev_ptr_x, dev_ptr_x + InputP.Nx * InputP.Ny * InputP.Nz, 0.0, thrust::plus<double>());
    			double change_y_sum = thrust::reduce(dev_ptr_y, dev_ptr_y + InputP.Nx * InputP.Ny * InputP.Nz, 0.0, thrust::plus<double>());
    			double change_z_sum = thrust::reduce(dev_ptr_z, dev_ptr_z + InputP.Nx * InputP.Ny * InputP.Nz, 0.0, thrust::plus<double>());
    			double change_x_avg = change_x_sum/(InputP.Nx * InputP.Ny * InputP.Nz);
    			double change_y_avg = change_y_sum/(InputP.Nx * InputP.Ny * InputP.Nz);
    			double change_z_avg = change_z_sum/(InputP.Nx * InputP.Ny * InputP.Nz);
    			
    			Conservation << <dimGrid3D, dimBlock3D >> > (Var.x_new, Var.y_new, Var.z_new, Var.b, change_x_avg, change_y_avg, change_z_avg, InputP);
    			/*-------------------For performance test-------------------*/
			cudaEventRecord(kernel_end, 0);
			cudaEventSynchronize(kernel_end);
			cudaEventElapsedTime(&kernel_time[2], kernel_start, kernel_end);
			...
			/*-------------------For performance test-------------------*/
			cudaEventRecord(kernel_start, 0);
			// Use regular swap since deactive blocks now use reduced evolution
			Swap_regular << <dimGrid3D, dimBlock3D >> > (Var.x_old, Var.x_new, Var.y_old, Var.y_new, Var.z_old, Var.z_new, InputP);
			cudaEventRecord(kernel_end, 0);
			cudaEventSynchronize(kernel_end);
